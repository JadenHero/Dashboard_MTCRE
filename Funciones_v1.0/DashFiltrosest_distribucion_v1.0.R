#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Funcion: DashFiltrosest_distribucion
# update: 24/04/2024
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
DashFiltrosest_distribucion_v1.0 <- function(equilibrios, observada, fecha, start, freq=4){
  
  # Transformar a objeto tipo matrix
  equilibrios <- as.matrix(equilibrios)
  print("--DashFiltrosest_distribucion: Se creo el objeto equilibrios tipo matrix")
  ...
  print("--DashFiltrosest_distribucion: Secuencia de fechas creada")
  
  # Verificar que la fecha selecionada este en la base
  # fecha = "2024-03-01"
  if(!(fecha %in% fechas.seq)){
    print("--DashFiltrosest_distribucion: La fecha solicitada sale del rango de la base")
  ...
  print("--DashFiltrosest_distribucion: Fecha especificada selecionda")
  
  # Tomar ITCR y equilibrios del trimestre seleccionado
  ITCR <- as.numeric(observada[tt])
  equilibrios <- equilibrios[tt, ]
  print("--DashFiltrosest_distribucion: Tomando ITCR y equilibrios de la fecha seleccionada")
  ...
  print("--DashFiltrosest_distribucion: Tomando el 95% central de la distribucion")
  
  
  # Media y Mediana
  mediana <- median(equilibrios)
  media <- mean(equilibrios)
  
  # Quantiles
  probs <- c(0.25,0.5,0.75)
  quantiles <- quantile(equilibrios, prob=probs)
  print("--DashFiltrosest_distribucion: Quantiles calculados")
  ...
  print("--DashFiltrosest_distribucion: Calculando densidad")
  ...
  fig <- plot_ly() %>% 
    ...
    layout(title = list(text=paste0("<b>", "Distribución de las tendencias de la TCR para ",fecha.tri, "</b>"), font=list(size=18)),
  ...
  print("--DashFiltrosest_distribucion: Grafico plotly realizado")
