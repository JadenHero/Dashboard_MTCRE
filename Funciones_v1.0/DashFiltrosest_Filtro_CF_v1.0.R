...
#'
#' Filtro_CF realiza el filtro de Descomposición estacional por Loess a las variables solicitadas y regresa una lista con los valores de la variable y su suavizamiento generada por el filtro.
#'
...
#' @param theta Numero de coeficientes de media móvil para el modelo de series temporales
#'
...
DashFiltrosest_Filtro_CF_v1.0<-function(datos, variables=names(datos)[-1],inicio,freq=4,cor_col=FALSE,dat_cor=NULL, c=freq, pl=NULL,pu=NULL,nfix=NULL,theta=1){
  # Manejar datos.
  library(dplyr)
  library(tidyverse)
  ...
  ex_CF<-vector("list",length = length(variables))
  names(ex_CF)<-variables
  for (i in variables) {
    ...
    data_cf<-cffilter(dta.ts,pl=pl,pu=pu,nfix=nfix,theta=theta)$trend #Valores suavizados por el filtro de Christiano-Fitzgerald

    colnames(data_cf)<-"Filtro Christiano-Fitzgerald"

    data<-cbind(data,as.data.frame(data_cf))

    ex_CF[[i]]<-data
    
    print('--Filtro de CF se estimo de forma correcta--')
